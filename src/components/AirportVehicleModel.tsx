...
export default function AirportVehicleModel({ vehicle, onClose }: VehicleModalProps) {
  ...
  const handleBooking = () => {
    if (!startDate || !endDate) {
      setErrorMessage('⚠️ Please select both start and end dates to proceed.');
      return;
    }

    setErrorMessage(''); // clear previous error
    const message = `Airport Taxi Service Booking Request\n\nHi! I'd like to book the ${vehicle.name} for an airport transfer.\n\nStart Date: ${startDate}\nEnd Date: ${endDate}\nCapacity: ${vehicle.capacity} Passengers`;
  ...
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="relative bg-slate-800 rounded-2xl max-w-4xl w-full flex flex-col md:flex-row max-h-[90vh] text-white shadow-2xl">
