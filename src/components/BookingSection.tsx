...
export const BookingSection = () => {
  ...
  const [errors, setErrors] = useState({ pickupLocation: '', dropoffLocation: '' });
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    updateFormData({ [name]: name === 'adults' || name === 'children' ? parseInt(value) : value });
  };
  ...
  const WHATSAPP_NUMBER = "YOUR_WHATSAPP_NUMBER"; // e.g., 94771234567
...
Please confirm availability and price.
          ...
          return (
            <React.Fragment key={stepItem.num}>
              ...
              >
                ...
                <span className={`mt-2 text-xs md:text-sm font-semibold ${isActive ? 'text-orange-500' : 'text-gray-600'} ${isCompleted ? 'group-hover:text-black' : ''}`}>
                  {stepItem.title}
                </span>
              </button>
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              <div className="form-group">
                ...
                <input type="text" name="pickupLocation" id="pickupLocation" placeholder="Enter a location" value={formData.pickupLocation} onChange={handleChange} className={`w-full p-3 border rounded-md ${errors.pickupLocation ? 'border-red-500' : 'border-gray-300'}`} />
              ...
              <div className="form-group">
                ...
                <input type="text" name="dropoffLocation" id="dropoffLocation" placeholder="Enter a location" value={formData.dropoffLocation} onChange={handleChange} className={`w-full p-3 border rounded-md ${errors.dropoffLocation ? 'border-red-500' : 'border-gray-300'}`} />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="form-group">
                ...
                <input type="date" name="pickupDate" id="pickupDate" value={formData.pickupDate} onChange={handleChange} className="w-full p-3 border border-gray-300 rounded-md" />
              </div>
              <div className="form-group">
                <label htmlFor="pickupTime" className="block text-sm font-medium text-gray-700 mb-1">Pickup Time</label>
                <input type="time" name="pickupTime" id="pickupTime" value={formData.pickupTime} onChange={handleChange} className="w-full p-3 border border-gray-300 rounded-md" />
              </div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="form-group">
                <label htmlFor="transferType" className="block text-sm font-medium text-gray-700 mb-1">Transfer Type</label>
                <select name="transferType" id="transferType" value={formData.transferType} onChange={handleChange} className="w-full p-3 border border-gray-300 rounded-md bg-white">
              ...
              <div className="form-group">
                ...
                <input type="number" name="adults" id="adults" min="1" value={formData.adults} onChange={handleChange} className="w-full p-3 border border-gray-300 rounded-md" />
              </div>
              <div className="form-group">
                <label htmlFor="children" className="block text-sm font-medium text-gray-700 mb-1">Children</label>
                <input type="number" name="children" id="children" min="0" value={formData.children} onChange={handleChange} className="w-full p-3 border border-gray-300 rounded-md" />
              </div>
            </div>
        ...
        return (
          <div className="space-y-6">
            <h3 className="text-xl font-semibold text-gray-800">
              Choose a Vehicle ({totalPassengers} {totalPassengers > 1 ? 'Passengers' : 'Passenger'})
            </h3>
              ...
              {(['all', 'car', 'van'] as const).map((cat) => (
                <button
                  ...
                  className={`capitalize py-2 px-4 rounded-md text-sm font-semibold transition-colors
                    ${filter === cat ? 'bg-orange-500 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}
                  `}
                >
                  {cat}s
                </button>
            ...
            {filteredVehicles.length === 0 ? (
              <p className="text-center text-gray-600 p-4 border rounded-md bg-gray-50">
                No vehicles available for {totalPassengers} passengers {filter !== 'all' ? `in the '${filter}' category` : ''}. 
                Please adjust passenger count or change the filter.
              </p>
            ) : (
                ...
                {filteredVehicles.map((vehicle) => (
                  <div
                    ...
                    className={`flex flex-col md:flex-row items-center border rounded-lg p-4 cursor-pointer transition-all
                      ${selectedVehicle?.id === vehicle.id ? 'border-orange-500 border-2 shadow-lg scale-[1.01]' : 'border-gray-200 hover:shadow-md'}
                    `}
                  >
                    <img src={vehicle.image} alt={vehicle.name} className="w-32 h-20 object-contain rounded-md mb-4 md:mb-0 md:mr-4" />
                    ...
                    <div className="md:ml-4 mt-4 md:mt-0 text-center">
                      <p className="font-bold text-lg text-orange-600">{vehicle.payment}</p>
                    </div>
                  </div>
                ))}
              </div>
        ...
        if (!selectedVehicle) {
          return (
            <div className="text-center p-4">
              <p className="text-red-600 font-semibold">No vehicle selected.</p>
              <button onClick={prevStep} className="mt-2 text-orange-500 hover:underline font-medium">
          ...
          <div className="space-y-6">
            ...
            <div className="p-4 border rounded-md bg-gray-50 space-y-3">
              <div>
                ...
                <p className="text-lg font-medium">{formData.pickupLocation}</p>
              </div>
              <div>
                <strong className="text-gray-700 text-sm">To:</strong>
                <p className="text-lg font-medium">{formData.dropoffLocation}</p>
              </div>
              <div className="grid grid-cols-2 gap-4 pt-2 border-t">
                <div>
                  <strong className="text-gray-700 text-sm">Date & Time:</strong>
                  <p>{formData.pickupDate || 'ASAP'} at {formData.pickupTime || 'N/A'}</p>
                </div>
                <div>
                  <strong className="text-gray-700 text-sm">Passengers:</strong>
                  <p>{formData.adults} Adults, {formData.children} Children</p>
                </div>
              </div>
              <div className="pt-3 border-t">
                <strong className="text-gray-700 text-sm">Vehicle:</strong>
                <p className="text-xl font-semibold text-orange-600">{selectedVehicle.name}</p>
                <p className="text-sm font-medium">{selectedVehicle.payment}</p>
              </div>
            </div>
            <p className="text-sm text-gray-600">
              Click the button below to send your booking request directly to us via WhatsApp. 
  ...
  const renderSummary = () => {
    return (
      <div className="bg-white rounded-lg shadow-md p-6 h-full">
        {step === 1 && (
          <>
            <h4 className="text-lg font-semibold mb-4 text-gray-800">Your Route</h4>
            ...
            <div className="mt-4 grid grid-cols-2 gap-4">
              <div className="bg-gray-50 p-3 rounded-md">
                <span className="text-sm text-gray-600 block">Total Distance</span>
        ...
        {step === 2 && (
          <>
            <h4 className="text-lg font-semibold mb-4 text-gray-800">Your Ride</h4>
            <div className="space-y-4">
              <div className="border-b pb-2">
                <strong className="text-gray-600 text-sm">From:</strong>
                <p className="font-medium text-gray-900">{formData.pickupLocation || 'Not set'}</p>
              </div>
              <div className="border-b pb-2">
                <strong className="text-gray-600 text-sm">To:</strong>
                <p className="font-medium text-gray-900">{formData.dropoffLocation || 'Not set'}</p>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <strong className="text-gray-600 text-sm">Passengers:</strong>
                  <p className="font-medium text-gray-900">{formData.adults} Adults, {formData.children} Children</p>
                </div>
                <div>
                  <strong className="text-gray-600 text-sm">Date:</strong>
                  <p className="font-medium text-gray-900">{formData.pickupDate || 'Not set'}</p>
                </div>
              </div>
            </div>
          </>
        ...
        {step === 3 && selectedVehicle && (
          <>
            <h4 className="text-lg font-semibold mb-4 text-gray-800">Your Vehicle</h4>
            <img 
              src={selectedVehicle.image} 
              alt={selectedVehicle.name} 
              className="w-full h-48 object-contain rounded-md mb-4 bg-gray-100 p-2" 
            />
            <h5 className="text-2xl font-bold text-gray-900">{selectedVehicle.name}</h5>
            <p className="text-orange-600 font-semibold text-lg">{selectedVehicle.payment}</p>
  ...
  // --- MAIN RENDER ---
  return (
    <section className="bg-gray-100 p-4 md:p-8 rounded-lg shadow-xl max-w-6xl mx-auto">
      <h2 className="text-3xl font-bold text-center mb-2 text-gray-900">Book Your Taxi Now</h2>
      <p className="text-center text-gray-600 mb-8">
        Get instant quotes and book your Ceylon Taxi service with our easy online booking system
      </p>
